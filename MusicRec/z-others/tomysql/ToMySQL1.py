...
class ToMySQL:
    ...
    def playListSingMessToMySQL(self):
        i=0
        for line in open("./data/p1_sing_id.txt","r",encoding="utf-8"):
            t=line.strip().split("\t")
            if len(t)<2:
                continue
...
if __name__ == "__main__":
    t1 = threading.Thread(target=tomysql.playListSingMessToMySQL)
    t2 = threading.Thread(target=tomysql.playListMessToMysql)
    t3 = threading.Thread(target=tomysql.playListTagMessToMySQL)
    t1.start()
    t2.start()
    t3.start()
